...
import (
	...
	ServerPref "sticker_board/application/server_pref"
	LogService "sticker_board/lib/log_service"
	"strconv"
	"time"
)

func InitializeOSS(app *iris.Application){
	stickerOSS := app.Party("/api/oss/v1")

	stickerOSS.Get("/get_signature", ApiMiddleware.AuthVersionMiddleware, ApiMiddleware.AuthAccountMiddleware, getOSSSignature)
	stickerOSS.Post("/notify_file_uploaded", notifyFileUploaded)
}
...
func notifyFileUploaded(ctx iris.Context){
	...
	byteAuthorization, err := getAuthorization(ctx.Request())
	if err != nil {
		ctx.ResponseWriter().WriteHeader(http.StatusBadRequest)
		return
	}
...
func getOSSKeyID() string {
	return OSSAlicloud.AccessKeyID
}

func getOSSKeySecret() string {
	return OSSAlicloud.AccessKeySecret
}

func getOSSHost() string {
	return "https://"+OSSAlicloud.BucketName+"."+OSSAlicloud.Endpoint
}

func getOSSCallbackHost() string {
	return "http://"+ServerPref.GetHostDomain()+":"+ServerPref.GetHostPort()+"/api/oss/v1/notify_file_uploaded"
}

func getOSSExpireTime() int64 {
	return 30
}
...
func get_policy_token(ctx iris.Context, ossDirectoryPath string) string {
	...
	h := hmac.New(func() hash.Hash { return sha1.New() }, []byte(getOSSKeySecret()))
	...
	policyToken.AccessKeyId = getOSSKeyID()
